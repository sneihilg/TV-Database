...
for i in range(0,len(Buffer_distance)):
    ...
    with open('Irregular_Ser_Cont_Points_Buffer_Distance%s.csv'%Buffer_distance[i], 'rb') as csvfile:
        with open('Columbus_Irregular_Ser_Cont_Points_GradeB_SepDis%s.csv'%Buffer_distance[i], 'w') as csvoutput:
            next(csvfile)
