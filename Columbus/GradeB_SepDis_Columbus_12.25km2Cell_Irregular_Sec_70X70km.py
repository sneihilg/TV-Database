...
for strip in range(0,int(Side_secon_net/Side_of_cell)):
    ...
    plt.title('Strip with %.1f km X %.1f km cells around Columbus over %d KmX %d Km area' %(Side_of_cell,Side_of_cell,Side_secon_net,Side_secon_net))
    plt.savefig('Strip with %.1f km X %.1f km across Columbus( %d Km X %d Km).png'%(Side_of_cell,Side_of_cell,Side_secon_net,Side_secon_net),dpi=300)
plt.show()
...
json.dump(out_channels, open('GradeB_SepDis_Columbus_%.1fkm2Cell_Irregular_Sec70X70km2.json'%(Side_of_cell),'w'))           
