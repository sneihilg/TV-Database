...
for strip in range(0,Side_secon_net/Side_of_cell):
    origin = geopy.Point(Lat_A, Lon_A)
    bearing_deg_A_to_D = 180
    d_A_to_D = Side_of_cell
    destination = VincentyDistance(kilometers=d_A_to_D).destination(origin,bearing_deg_A_to_D)
    ...
    val_D = (Lon_D,Lat_D)
    rt_D.append(val_D)
    for i in range(0,Side_secon_net/Side_of_cell):
...
for strip in range(0,Side_secon_net/Side_of_cell):    
    for k in range(0,Side_secon_net/Side_of_cell):
    ...
    plt.title('Strip with %d km X %d km cells around Columbus over %d KmX %d Km area' %(Side_of_cell,Side_of_cell,Side_secon_net,Side_secon_net))
    plt.savefig('Strip with %d km X %d km across Columbus( %d Km X %d Km).png'%(Side_of_cell,Side_of_cell,Side_secon_net,Side_secon_net),dpi=300)
plt.show()
...
df = pd.read_csv('Columbus_Irregular_Ser_Cont_Points_GradeB_SepDis.csv')
...
json.dump(out_channels, open('GradeB_SepDis_Columbus_%dkm2Cell_Irregular_Sec70X70km2.json'%(Side_of_cell),'w'))           
