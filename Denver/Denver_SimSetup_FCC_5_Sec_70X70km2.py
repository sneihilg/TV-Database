...
Data = json.load(open('GradeB_Denver_%dkm2Cell_Irregular_Sec70X70km2.json'%(Side_of_cell),'r'))
...
json.dump(out_channels, open('GrdB_Denver_%dkm2_Sim_Setup_Sec70X70km2.json'%(Side_of_cell),'w'))           


Data = json.load(open('GrdB_Denver_%dkm2_Sim_Setup_Sec70X70km2.json'%(Side_of_cell),'r'))
...
for cell in range(0,len(Data)):
    ...
    out = {"cell_center":Cell_center,
           ...
           "Origin_xy":(Data[str(cell)]['Origin_xy'])}
    out_channels[cell] = out
json.dump(out_channels, open('GrdB_Denver_%dkm2_Sim_Data_Sec_70X70km2.json'%(Side_of_cell),'w')) 
