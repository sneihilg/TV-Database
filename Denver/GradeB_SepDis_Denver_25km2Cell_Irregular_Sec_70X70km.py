...
for strip in range(0,Side_secon_net/Side_of_cell):    
    ...
    plt.title('Strip with %d km X %d km cells around Denver over %d KmX %d Km area' %(Side_of_cell,Side_of_cell,Side_secon_net,Side_secon_net))
    plt.savefig('Strip with %d km X %d km across Denver( %d Km X %d Km).png'%(Side_of_cell,Side_of_cell,Side_secon_net,Side_secon_net),dpi=300)
plt.show()
...
df = pd.read_csv('Denver_Irregular_Ser_Cont_Points_GradeB_SepDis.csv')
...
json.dump(out_channels, open('GradeB_SepDis_Denver_%dkm2Cell_Irregular_Sec70X70km2.json'%(Side_of_cell),'w'))           
