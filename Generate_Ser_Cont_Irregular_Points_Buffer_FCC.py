#Created: Sep 16,2016. Generate separation distance points (when it is irregular) around TV tower to use it to get an estimate of available channels.
...
Buffer_distance = np.arange(11.1, 41.1, 1)
#Create a csv for secondary antenna height 10 m and irregular Grade B service contour
for i in range(0,len(Buffer_distance)):
    j=0
    with open('TV_TOWER_DATA_FINAL.csv', 'rb') as csvfile:
        with open('Irregular_Ser_Cont_Points_Buffer_Distance%s.csv' %Buffer_distance[i], 'w') as csvoutput:
            next(csvfile)
            ...
            for row in tv_reader:
                ...
                if row[16] in (None,""):
                    ...
                    for ls in range(360):
                        ...
                        destination = VincentyDistance(kilometers=Buffer_distance[i]).destination(origin,bearing)
